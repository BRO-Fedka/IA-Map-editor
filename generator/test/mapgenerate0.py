...
def generatePolygon2(p,pix) :
    ...
    for _ in range(0,len(p)):
        points.append(p[_])
        d = (math.sqrt((p[int((_ + 1) % len(p))][0]-cx)**2+(p[int((_ + 1) % len(p))][1]-cy)**2)+math.sqrt((p[_][0]-cx)**2+(p[_][1]-cy)**2))/2
...
def smoothPolygon(p,pix):
    points = []
    for _ in range(0,len(p)):
        a0 = p[_][0]+((((p[(len(p)+_-1)%len(p)][0]+p[_][0])/2+( p[(_+1)%len(p)][0]+p[_][0])/2)/2)-p[_][0])*pix
        a1 = p[_][1]+((((p[(len(p)+_-1)%len(p)][1]+p[_][1])/2+( p[(_+1)%len(p)][1]+p[_][1])/2)/2)-p[_][1])*pix
        points.append([a0,a1])
    return points
def main():
    random.seed(0)
    ...
    for _ in range(0,10000):
        ...
        #     dis = 100*random.random()
            poly = generatePolygon(2+random.random()*28,2+random.random()*28,random.random()*5,0.5,0.5,random.randint(10,30)) #128+math.cos(deg)*dis,128+math.sin(deg)*dis
    ...
    for x in range(0,WH):
        for y in range(0, WH):
            MAP['Q'][(x,y)] = {'p':Polygon([[x,y],[x+1,y],[x+1,y+1],[x,y+1]]),'B':[],'Z':[],'G':[],'S':[]}
    ...
    for _ in range(0,int(32*32)):
            try:
                #     deg = random.random() * math.pi*2
                #     dis = 100*random.random()
                X = 2 + random.random() * 28
                Y= 2 + random.random() * 28
                poly = generatePolygon(X, Y, 0.1+random.random() * 0.1, 0.75,0.75, random.randint(6, 8))  # 128+math.cos(deg)*dis,128+math.sin(deg)*dis
                ...
                logging.exception("message")
                    # print(_0)
                    # print(x,y)
    pickle.dump(MAP, open('MAPEXPS0', 'wb'))
