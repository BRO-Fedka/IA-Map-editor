...
class BaseCity:
    def __init__(self, island, seg):
        ...
        if not (not self.city_foundation.intersects(self.island.world.MAP_SQUARE_LS) and self.city_foundation.intersects(
    ...
    def save(self,data):
        ...
        data['C'].append(list(map(lambda v: list(map(lambda g: round(g*self.island.world.WH, 2), v)), self.lpier.exterior.coords[:])))
        data['C'].append(list(map(lambda v: list(map(lambda g: round(g*self.island.world.WH, 2), v)), self.rpier.exterior.coords[:])))
        for p in self.piers:
            data['C'].append(list(map(lambda v: list(map(lambda g: round(g*self.island.world.WH, 2), v)), p.exterior.coords[:])))
