...
class MapGrid:
    ...
    __WH: int = 0
    __vertical_lines: List[int] = []
    __horizontal_lines: List[int] = []
    ...
    def update(self):
        self.__workspace.coords(self.__border_id, self.__workspace.calc_x(0), self.__workspace.calc_y(0),self.__workspace.calc_x(self.__WH), self.__workspace.calc_y(self.__WH))
        while len(self.__vertical_lines) < self.__WH-1:
            self.__vertical_lines.append(self.__workspace.create_line(0, 0, 0, 0, fill="red", width=1))
        while len(self.__horizontal_lines) < self.__WH-1:
            self.__horizontal_lines.append(self.__workspace.create_line(0, 0, 0, 0, fill="red", width=1))
        for i in range(0,len(self.__horizontal_lines)):
            line_x = i+1
            if i >= self.__WH:
                line_x = self.__WH
            self.__workspace.coords(self.__horizontal_lines[i], self.__workspace.calc_x(line_x), self.__workspace.calc_y(0),self.__workspace.calc_x(line_x), self.__workspace.calc_y(self.__WH))
        for i in range(0,len(self.__vertical_lines)):
            line_y = i+1
            if i >= self.__WH:
                line_y = self.__WH
            self.__workspace.coords(self.__vertical_lines[i], self.__workspace.calc_x(0), self.__workspace.calc_y(line_y),self.__workspace.calc_x(self.__WH), self.__workspace.calc_y(line_y))
